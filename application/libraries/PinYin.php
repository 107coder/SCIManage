...
* 使用方法:
*  $py = new PinYin();
*  $all_py = $py->get_all_py("吴国伟"); //输出 ['wu','guo','wei']，输出字符串调用join方法,join('',$all_py)
*  $first_py = $py->get_first_py($all_py);//输出 wgw
*  $first_letter = $py->get_first_letter($all_py);// 输出 w
*
* +------------------------------------------------------
*/
class PinYin
{
    ...
    */
    ...
    /**
     * 根据中文姓名返回一个用逗号分隔的字符串，  姓,名
     * @param $name
     * @return string|null
     */
    public function get_name_string($name){
        $nameArr = $this->get_all_py($name);
        if(in_array('',$nameArr)){
            return null;
        }
        $nameStr = '';
